...
//        }
        var queryString = ""
        if parameters != nil {
            queryString = self.stringFromParameters(parameters! as! Dictionary<String,String>)
        }
...
//        }
        ...
        for (key,value) in params {
            if value is HTTPUpload {
                return true
            } else if let subParams = value as? Dictionary<String,AnyObject> {
    ...
    func stringFromParameters(parameters : [String: String]) -> String {
        var arrayOfStrings: [String] = []
        for (key, value) in parameters {
            let parameter = "\(key.stringByAddingPercentEscapesUsingEncoding(NSUTF8StringEncoding)!)=\(value.stringByAddingPercentEscapesUsingEncoding(NSUTF8StringEncoding)!)"
            arrayOfStrings.append(parameter)
        }
//        return "&".join(arrayOfStrings)
        return arrayOfStrings.joinWithSeparator("&")
    }
