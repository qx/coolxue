...
class ProductHeadView:UIView,UIScrollViewDelegate{
    ...
    func scrollViewDidScroll(scrollView: UIScrollView) {
        ...
        let currPage = (self.scrollView.contentOffset.x + self.scrollView.frame.width*0.5)/self.scrollView.frame.width
    ...
    func loadImage(list:Array<Vedio>){
        ...
        print("imgViewW = \(imgViewW) imgViewH = \(imgViewH)")
        for(var i:Int = 0;i<list.count;i++){
            let movePo:Vedio = list[i]
...
//            xibView!.imageView.addSubview(blurView)
            ...
            let bn:UIButton = UIButton(type:UIButtonType.Custom)
    ...
    func showAlbumInfo(sender:UIButton){
        ...
        let channel:Vedio = ProductHeadView.topAlbumList![id]
