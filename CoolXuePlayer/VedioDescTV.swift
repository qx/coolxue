...
class VedioDescTV: UITableView,UITableViewDataSource,UITableViewDelegate {
    ...
    override func awakeFromNib() {
        if self.delegate == nil {
            let nib1 = UINib(nibName: "VedioDescTVCell", bundle: nil)
            let nib2 = UINib(nibName: "VedioListTabVCell", bundle: nil)
    ...
    func tableView(tableView: UITableView, cellForRowAtIndexPath indexPath: NSIndexPath) -> UITableViewCell {
        if indexPath.section == 0 {
            let cell = tableView.dequeueReusableCellWithIdentifier("VedioDescTVCellID", forIndexPath: indexPath) as? VedioDescTVCell
            ...
            let vedio = self.relatedAlbumList[indexPath.row] as Vedio
            ...
            let cell = tableView.dequeueReusableCellWithIdentifier("VedioListTabVCellID", forIndexPath: indexPath) as? VedioListTabVCell
            let vedio = self.relatedVedioList[indexPath.row] as Vedio
            cell!.nameLabel.text = vedio.name
            cell!.authorLabel.text = "作者："+vedio.author
            cell!.palyTimesLabel.text = "播放次数：\(vedio.playTimes)"
            cell!.playCostLabel.text = "爱苦逼：\(vedio.playCost)"
            ...
            let cell = tableView.dequeueReusableCellWithIdentifier("VedioListTabVCellID", forIndexPath: indexPath) as? VedioListTabVCell
            let vedio = self.relatedBookList[indexPath.row] as Vedio
            cell!.nameLabel.text = vedio.name
            cell!.authorLabel.text = "作者："+vedio.author
            cell!.palyTimesLabel.text = "播放次数：\(vedio.playTimes)"
            cell!.playCostLabel.text = "爱苦逼：\(vedio.playCost)"
    ...
    func tableView(tableView: UITableView, didSelectRowAtIndexPath indexPath: NSIndexPath) {
        ...
        if indexPath.section == 1 && self.gotoViewDelegate != nil {
            let vedio:Vedio = self.relatedAlbumList[indexPath.row]
            self.gotoViewDelegate!.turnToVedioListView(vedio)
        //相关视频
        }else if indexPath.section == 2 {
            let vedio:Vedio? = self.relatedVedioList[indexPath.row]
            if vedio != nil {
                self.cuobj = vedio
                //获取视频播放地址
                let url = "http://www.icoolxue.com/video/play/url/"+String(vedio!.id)
