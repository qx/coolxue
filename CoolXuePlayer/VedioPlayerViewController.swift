...
class VedioPlayerViewController: UIViewController,VedioDescTVDelegate {
    ...
    func ReceivedNotif(obj:NSNotification){
        let vedio:Vedio! = obj.object as! Vedio
        if vedio != nil {
            self.channel = vedio
            self.initVedio()
        }
    ...
    func playByItem(url:String){
        ...
        do {
            try  nurl?.setResourceValues(["referer":"http://www.icoolxue.com"])
        } catch {
            print(error)
        }
    ...
    override func observeValueForKeyPath(keyPath: String?, ofObject object: AnyObject?, change: [String : AnyObject]?, context: UnsafeMutablePointer<Void>) {
        ...
        if keyPath == "loadedTimeRanges" {
            ...
            //print("startSeconds=\(startSeconds) durationSeconds=\(durationSeconds)")
            ...
            //print("loadedTimeRanges currdownDuration=\(currdownDuration)totalDuration=\(totalDuration)")
            //显示缓冲进度
            ...
            if tempplayerItem.status == AVPlayerItemStatus.ReadyToPlay {
                ...
                playerLayer.player!.addPeriodicTimeObserverForInterval(CMTimeMake(1, 1), queue: nil, usingBlock: { (currplaytime:CMTime) -> Void in
                    //print("addPeriodicTimeObserverForInterval")
...
//           }
    ...
    func updateVideoSlider(){
        ...
        if tempplayeritem != nil {
            ...
            //print("currplayerSecond = \(currplayerSecond) totalplayerSecond=\(totalplayerSecond)")
        }
    }
    
    func convertTime(second:NSTimeInterval)->String{
        //print("convertTime time =\(second)")
        let d:NSDate = NSDate(timeIntervalSince1970: second)
        ...
        let showtimeNew = formatter.stringFromDate(d)
        
        return showtimeNew;
    }
    
    @IBAction func bnfullScreenClicked(sender: AnyObject) {
        print("bnfullScreenClicked")
    ...
    @IBAction func playSliderValueChange(sender: UISlider) {
        if playerLayer.player != nil && playerLayer.player!.status == AVPlayerStatus.ReadyToPlay {
            ...
            //print("dragedSeconds:\(dragedSeconds)");
